...
├── security/
│   ├── docker-scout-convert.sh            # Converts Docker Scout SARIF to Sonar format (CI)
│   ├── zap-scan.sh                        # OWASP ZAP security scanning (CD)
│   ├── zap-convert-to-sonar.sh            # Converts ZAP SARIF to Sonar format (CD)
│   └── zap-upload-to-sonar.sh             # Uploads ZAP results to SonarQube (CD)
└── utils/
...
### Security Scanning
- **docker-scout-convert.sh**: Converts Docker Scout SARIF to SonarQube format (CI stage)
- **zap-scan.sh**: Runs OWASP ZAP security scan against deployed applications (CD stage)
- **zap-convert-to-sonar.sh**: Converts ZAP SARIF to SonarQube format (CD stage)
- **zap-upload-to-sonar.sh**: Uploads ZAP results to SonarQube (CD stage)
