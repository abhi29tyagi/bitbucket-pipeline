...
if [[ "${1:-}" == "--install-cron" ]]; then
  ...
  INSTALL_DIR="/usr/local/lib/shared-pipelines/scripts/traefik"
  ...
  if command -v sudo >/dev/null 2>&1; then
    ...
    sudo ln -sf "$STABLE_PATH" /usr/local/bin/certbot-cloudflare-renew
  else
    mkdir -p "$INSTALL_DIR"
    cp "$SCRIPT_PATH" "$STABLE_PATH"
    chmod +x "$STABLE_PATH"
    ln -sf "$STABLE_PATH" /usr/local/bin/certbot-cloudflare-renew 2>/dev/null || true
  fi
