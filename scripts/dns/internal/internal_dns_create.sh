...
if [ -n "${BITBUCKET_PR_ID:-}" ]; then
    ...
    TARGET_IP="${PREVIEW_TARGET_IP}"
    echo "Creating preview A record: ${FULL_DOMAIN} -> ${TARGET_IP}"
else
    # Dev deployment - use environment-specific variables
    : "${DEV_TARGET_IP:?Missing DEV_TARGET_IP for dev deployment}"
    ...
    echo "Creating dev A record: ${FULL_DOMAIN} -> ${TARGET_IP}"
fi
